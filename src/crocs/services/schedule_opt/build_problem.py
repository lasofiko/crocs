"""
Постановка задачи назначения смен.

Полный перечень допустимых смен (бинарные переменные в MILP) строится здесь без усечения:
каждая комбинация сотрудник × день × станция × шаблон длительности × допустимый старт в окне sched.
"""
...
def clock_hours(val: object) -> float:
    if pd.isna(val):
        return float("nan")
...
def _ds_norm_ns(ds: Any) -> int:
    """Стабильный int-ключ даты без повторных объектов Timestamp в множестве."""
    return int(pd.Timestamp(ds).normalize().value)


def dedupe_shift_options(options: list[ShiftOption]) -> list[ShiftOption]:
    seen: set[tuple[Any, ...]] = set()
    out: list[ShiftOption] = []
    for o in options:
        key = (
            o.emp_key,
            o.day_idx,
...
def parse_shifts(shifts: pd.DataFrame) -> list[tuple[int, int]]:
    sh = shifts.copy()
...
def sched_windows(sched: pd.DataFrame) -> dict[tuple[str, int], tuple[float, float]]:
    s = sched.copy()
    ...
    for _, row in s.iterrows():
        ...
        lo = clock_hours(row[stcol])
        hi = clock_hours(row[ftcol])
...
def demand_grid(
    ...
    day_idx_of: dict[Any, int] = {d: i for i, d in enumerate(days)}
    demand: dict[tuple[int, int, str], int] = {}
    for _, row in agg.iterrows():
        d = row["ds"]
        day_idx = day_idx_of[d]
...
class ShiftAssignmentProblem:
    ...
    # Один раз при сборке задачи (не дергать objective_coeff()/duration на каждом ограничении).
    objective_coeffs: list[int]
    shift_duration_hours: list[int]


def build_shift_assignment_problem(inputs: SchedulingInputs) -> ShiftAssignmentProblem:
    ...
    days, hours, stations, demand_raw, day_ts = demand_grid(inputs.hourly_demand)
    demand = demand_raw
    ...
    roster_keys = sorted({nid(x) for x in sched_df[ecol].dropna()}, key=lambda x: x)
    roster_display: dict[str, Any] = {}
    ...
    station_pen = station_penalties(inputs.station_priorities, roster_keys, stations)
        ...
        for day_idx in range(len(days)):
            ...
            for st in stations:
                ...
                for dur, spr in shift_pairs:
                    if dur > smax + 1e-6:
                        continue
                    start_min = max(open_h, int(math.ceil(win_lo - 1e-9)))
                    start_max = min(
                        rest_end_exc - dur,
                        int(math.floor(win_hi - dur + 1e-9)),
                    )
                    if start_min > start_max:
                        continue
                    for start_h in range(start_min, start_max + 1):
                        end_exc = start_h + dur
    ...
    options = dedupe_shift_options(options)
    ...
    hours_arr = np.asarray(hours, dtype=np.int32)

    for i, opt in enumerate(options):
        by_emp_day[(opt.emp_key, opt.day_idx)].append(i)
        by_emp[opt.emp_key].append(i)
        sh, dur = opt.start_h, opt.duration
        mask = (hours_arr >= sh) & (hours_arr < sh + dur)
        for hour in hours_arr[mask]:
            coverage_idxs[(opt.day_idx, int(hour), opt.station)].append(i)
    ...
    obj_coeffs = [o.objective_coeff() for o in options]
    dur_hours = [o.duration for o in options]
    return ShiftAssignmentProblem(
        options=options,
        ...
        shift_duration_hours=dur_hours,
    )
