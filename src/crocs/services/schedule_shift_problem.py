"""
Общая постановка задачи назначения смен: генерация допустимых колонок (смен) и индексов покрытия.

Используется CP-SAT и Pyomo+MILP; при каждом solve постановка строится заново через `build_shift_assignment_problem`.
"""
...
# Обратная совместимость для импортов вида _demand_grid
